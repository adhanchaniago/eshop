...
class Item extends CI_Controller {
	...
	{
		parent::__construct();
		$this->load->library(array('template', 'form_validation')); 	//part3
      $this->load->model('admin');	//part3 pemanggilan dari model
	}

   public function index() //part5
   {
		$this->cek_login();
   ...
   public function add_item()	//part3 awal
   {
		$this->cek_login();
      ...
      if ($this->input->post('submit', TRUE) == 'Submit') {
         ...
         $this->form_validation->set_rules('desk', 'Deskripsi', 'required|min_length[4]');//part 3 akhir

         if ($this->form_validation->run() == TRUE) //part 3 awal lagi
         {
...
//part 5
	...
	public function update_item()
   {
      ...
      if ($this->input->post('submit', TRUE) == 'Submit') {
         ...
         $this->form_validation->set_rules('berat', 'Berat Item', 'required|numeric');
         $this->form_validation->set_rules('status', 'Status', 'required|numeric');
         $this->form_validation->set_rules('desk', 'Deskripsi', 'required|min_length[4]'); //part3 selesai

         if ($this->form_validation->run() == TRUE) //part4
         {
				$config['upload_path'] = './assets/upload/';
				$config['allowed_types'] = 'jpg|png|jpeg';
				$config['max_size'] = '2048';
				...
				if ($this->upload->do_upload('foto'))
				{
					//fetch data file yang diupload ( part4 )
					$gbr = $this->upload->data();
